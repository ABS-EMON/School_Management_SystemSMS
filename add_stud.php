			...
			<div id="section">
				...
				<div class="content">
					...
					<?php
						if(isset($_POST["submit"]))
						{
							...
							{
								$sq="insert into student_login(SGPA,SRNO,SPASS,SNAME,SFNAME,SDOB,SGEN,SPNO,SMAIL,SPADDR,SCLASS,SSEC,SIMG,TID) values('{$_POST["gpa"]}','{$_POST["rno"]}','{$_POST["pass"]}','{$_POST["name"]}','{$_POST["fname"]}','{$edate}','{$_POST["gen"]}','{$_POST["pho"]}','{$_POST["email"]}','{$_POST["addr"]}','{$_POST["cla"]}','{$_POST["sec"]}','{$target_file}','{$_SESSION["TID"]}')";
				...
				<div class="lbox">
					<label> ID</label><br>
						<?php
							$no="S101";
							$sql="select * from student_login order by SID desc limit 1";
							$res=$db->query($sql);
							if($res->num_rows>0)
							{
								$row1=$res->fetch_assoc();
								$no=substr($row1["SRNO"],1,strlen($row1["SRNO"]));
								$no++;
								$no="S".$no;
							}
					...
					<input type="text" class="input3" name="pass"><br><br>
					...
					<input type="text" class="input3" maxlength="15" name="pho"><br><br>
				</div>
				...
				<label> Parent's Mail Id</label><br>
					...
					<label> CGPA </label><br>
                    <input type="number" step="0.01" class="input3" name="gpa"><br><br>
